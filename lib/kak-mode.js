...
export default {
  ...
  activate (state) {
    ...
    const runCommands = (key, { modifier } = {}) => {
      ...
      const selectionCommand = selectionCommands[key]
      const editorCommand = editorCommands[key]
      if (selectionCommand) {
        // selections commands
        const selections = editor.getSelections()
        selections.map(selection => selectionCommand(selection, commandAuxArg))
      }
      if (editorCommand) {
        // editor commands
        editorCommand(editor, commandAuxArg)
      }
    }

    // IPC and hotwords handling
    const handleHotword = hotword => {
      const hotwordsToKeys = {
        // block: ['m'],
        // yes: ['u', { modifier: 'shift' }],
        // no: ['u'],
        'select-all': '%',
        uppercase: '~',
      }
      const commandArgs = hotwordsToKeys[hotword]
      console.log('hotword args', commandArgs)
      if (commandArgs) {
        runCommands(...commandArgs)
      }
    }

    ipc.connectTo('hotwordListener', () => {
      ipc.of.hotwordListener.on('connect', () => {
        console.log('connected to IPC')
        ipc.of.hotwordListener.emit('app.register', {
          id: ipc.config.id,
        })
      })
      ipc.of.hotwordListener.on('app.hotword', ({ message }) => {
        console.log(message)
        handleHotword(message.trim())
      })
    })
