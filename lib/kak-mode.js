  ...
  activate (state) {
    ...
    const getKeyHandler = (key, modifier) => ({
      ...
      didDispatch: e => {
        ...
        if (searchPattern.searchSelectionsInitRanges) {
          if (escapePressed || enterPressed) {
            this.updateSearchPattern(INIT_STATE.searchPattern)
          } else {
        ...
        if (equals(mode, MODES.INSERT)) {
          ...
          e.abortKeyBinding()
          escapePressed && this.changeModeToNormal()
        } else {
    ...
    this.subscriptions.add(
      editor.onWillInsertText(({ text, cancel }) => {
        if (this.state.searchPattern.searchSelectionsInitRanges) {
          this.updateSearchPattern({
            pattern: text,
          })
          cancel()
