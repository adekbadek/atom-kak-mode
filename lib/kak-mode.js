...
import { editorCommands, selectionCommands, cursorCommands } from './commands/index'
...
export default {
  ...
  activate (state) {
    ...
    const getKeyHandler = (key, modifier) => ({
      ...
      didDispatch: e => {
        if (this.state.mode === MODES.INSERT) {
          // abortKeyBinding has to be called so that the event is propagated
          e.abortKeyBinding()
          key === 'escape' && this.changeModeToNormal()
        } else {
          const editor = atom.workspace.getActiveTextEditor()
          const commandAuxArg = { instance: this, modifier, count: this.state.count }

          const runCommands = (commands, getter) => {
            const command = commands[key]
            if (command) {
              if (getter) {
                editor[getter]().map(entity => command(entity, { editor, ...commandAuxArg }))
              } else {
      ...
      ...keys.map(char =>
        atom.commands.add(ATOM_TEXT_EDITOR, {
          ...
          ...!contains(char, uppercaseKeys) && {
            [`kak-mode:set-input-char-${char}-shift`]: getKeyHandler(char, 'shift'),
          },
        })
      )
    )
