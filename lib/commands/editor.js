...
export default {
  ...
  /**
   * @goto if a count is set, go to the corresponding line – otherwise enter goto mode or go to the first line if in goto mode already
   */
  g: withGoto(
    ...
    (_, { instance, editor, count }) => {
      if (count > 0) {
        const position = new Point(count, 0)
        editor.setCursorBufferPosition(position)
        editor.unfoldBufferRow(count)
        instance.updateCount(0)
      } else {
        instance.updateGoTo(true)
      }
    }
  ),
  /**
