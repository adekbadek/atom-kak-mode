...
const MATCHING_PAIRS_FOR_LOOKUP = { ...MATCHING_PAIRS, ...zipObj(closing, opening) }
...
export default (cursor, { editor, modifier }) => {
  ...
  buffer.scanInRange(getCharsRegex(ALL_MATCHING_CHARS), rangeAfterCursor, ({ range, matchText }) => {
    ...
    buffer[method](
      new RegExp(getCharsRegex([matchText, MATCHING_PAIRS_FOR_LOOKUP[matchText]]), 'g'),
      scanRange,
